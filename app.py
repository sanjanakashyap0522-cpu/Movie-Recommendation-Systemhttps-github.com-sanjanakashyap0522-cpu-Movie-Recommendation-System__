...
# Agar login nahi hua hai to black screen + login form dikhao
if not st.session_state["logged_in"]:
    ...
    st.markdown('<div class="login-box"><h1>🔐 Login Required</h1></div>', unsafe_allow_html=True)
    ...
    if st.button("Login"):
        if email == VALID_EMAIL and password == VALID_PASSWORD:
            ...
            st.success("✅ Login successful! Scroll down to explore the site.")
        else:
            st.error("❌ Invalid email or password.")
else:
    # ✅ Agar login successful hai to tumhara main app chalega
    # 👇 Yaha tum apna pura Movie Recommendation System ka code paste karo
    st.title("🎬 Movie Recommendation System")
    st.write("Welcome to Movieverse! You are logged in.")
    # Tumhara pura recommender code yaha daalna hai
...
# Sidebar
st.sidebar.title("🔥🎬 Welcome to MOVIEVERSE")
...
page = st.sidebar.radio(
    "📃 Pages",
    ["🏠Home", "❤️Favorites", "🔍 Search Movies", "🎯 Recommend Movies", "🎭 Explore by Genre", "🔥 Trending", "⭐ Top Rated"]
)
...
if page == "🏠Home":
    ...
    if len(st.session_state["favorites"]) == 0:
        ...
        # Show only titles in a dropdown
        fav_titles = [fav['title'] for fav in st.session_state["favorites"]]
        selected_fav = st.selectbox("Choose a movie to view details:", fav_titles)

        # Show details only when user selects
        if selected_fav:
...
if st.sidebar.button("Submit Feedback"):
    st.sidebar.success("Thanks for sharing your thoughts! ♥️")
